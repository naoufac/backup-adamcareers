...
/* ------------------------------------------------------------------ *
 * LLM client — OpenRouter primary, Z.AI fallback.
 *
 * Primary:   OpenRouter (OPENROUTER_API_KEY + OPENROUTER_MODEL)
 * Fallback:  Z.AI GLM (ZAI_API_KEY + ZAI_BASE_URL + ZAI_MODEL)
 *
...
function buildProviders(): Provider[] {
  ...
  if (orKey) {
    providers.push({
      ...
      model: orModel,
      extraHeaders: {
        "HTTP-Referer": process.env.OPENROUTER_HTTP_REFERER ?? "https://adamcareers.com",
  ...
  if (providers.length === 0) {
    console.warn("[llm] No LLM provider configured (OPENROUTER_API_KEY or ZAI_API_KEY).");
  }
